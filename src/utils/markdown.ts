...
export function renderMarkdown(content: string, options: RenderMarkdownOptions = {}): string {
  ...
  return addHeadingIds(simpleMarkdownToHtml(normalized));
}

export function extractTableOfContents(content: string, options: RenderMarkdownOptions = {}): TableOfContentsItem[] {
...
function addHeadingIds(html: string): string {
  ...
  return html.replace(/<h([1-6])([^>]*)>([\s\S]*?)<\/h\1>/gi, (_match, level, attributes, body) => {
...
function normalizeRelativeImages(content: string, assetBasePath?: string): string {
  ...
  return content.replace(/!\[([^\]]*)\]\((?!https?:\/\/|\/)([^)]+)\)/g, (_match, alt, src) => {
...
function createUniqueHeadingId(text: string, counts: Map<string, number>): string {
  const base = text.toLowerCase().replace(/[^\w\s-]/g, "").trim().replace(/\s+/g, "-").slice(0, 50) || "section";
